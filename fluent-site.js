/* Fluent Welding - shared navigation, drawing filters, RFQ UI and analytics events.
   Existing Apps Script endpoint and item field names are preserved.
   Google Analytics events fire only when gtag is available.
   No API keys or client-side credential storage. */
...
(() => {
  ...
  /* Track useful lead/contact clicks site-wide.
     No customer-entered personal information is sent. */
  ...
  function setMenu(group, open, mode = '') {
    ...
    if (open) {
      groups.forEach(other => {
        if (other !== group) setMenu(other, false);
      });
    }
  ...
  function closeMenus() {
    groups.forEach(group => setMenu(group, false));
  }
  ...
  if (nav && mobileToggle) {
    ...
    document.addEventListener('keydown', event => {
      ...
      if (openGroup) {
        const restore =
          openGroup.contains(document.activeElement);
  ...
  // Drawing content is present in the HTML and remains usable
  // without JavaScript.
  ...
  if (drawingSearch) {
    ...
    const filter = () => {
      const search =
        drawingSearch.value.trim().toLowerCase();
  ...
  // Referral intake uses the existing field names,
  // not a new API or partner contact list.
  ...
  function initServiceRequest() {
    ...
    const aerialTypes = new Set([
      'Drone Inspection Imagery',
      'Aerial Mapping / Site Survey',
      'Aerial Photography / Video'
    ]);

    const supported = (field, value) =>
      [...field.options].some(
        option => option.value === value
      );

    const clean = (id, limit = 500) =>
      (
    ...
    referral.addEventListener(
      ...
      async event => {
        ...
        if (
          ...
          setStatus(
            'Local preview only: no request was sent. Use the published website to request a connection.',
            'warning'
          );
        ...
        setStatus(
          'Sending the connection request to Fluent for processing...'
        );
        ...
        try {
          // Keep the backend schema exactly compatible
          // with the existing RFQ endpoint.

          // A quantity of one represents one inquiry,
          // not an order for a fabricated item.
          ...
          const project =
            referral.elements.project.value.trim();
          ...
          payload.set(
            'item_size_0',
            'Service referral only - no fabricated component requested'
          );
          ...
          payload.set(
            'item_notes_0',
            'SERVICE CONNECTION REQUEST - NOT AN ORDER\n' +
              'Service requested: ' +
              type.value
          );
          ...
          /*
             ANALYTICS:
             Browser successfully handed off the request.
             Because the request uses no-cors, this does not
             prove that Apps Script processed the request.
          */
          ...
          // The opaque response cannot verify processing
          // or email delivery. Retain entered data.
  ...
  const itemTemplate =
    itemList.firstElementChild.cloneNode(true);
  ...
  let rfqStartedTracked = false;

  form.addEventListener('input', () => {
    if (rfqStartedTracked) return;

    rfqStartedTracked = true;

    trackEvent(
      'rfq_started',
      {
        lead_type: 'website_rfq'
      }
    );
  });
  ...
  // Prefill only supported option values.
  // User-supplied text is assigned as text/value,
  // never HTML.

  const params =
    new URLSearchParams(location.search);

  function selectSupported(
    field,
    value
  ) {
    if (
  ...
  if (params.has('size')) {
    form.elements.item_size_0.value =
      params
        .get('size')
        .slice(0, 200);
  }
  ...
  form.elements.item_notes_0.value =
    [
      part
  ...
  if (part.endsWith('-304')) {
    selectSupported(
      form.elements.item_material_0,
      '304 Stainless'
    );
  }
  ...
  form.addEventListener(
    ...
    async event => {
      ...
      // Local review must never send
      // accidental quote requests.
      ...
      try {
        ...
        /*
           GOOGLE ANALYTICS RFQ CONVERSION EVENT

           This fires only after the browser successfully
           hands the request to the Google Apps Script endpoint.

           Because the request uses no-cors, the browser cannot
           verify server-side processing or email delivery.
           The RFQ confirmation email remains the actual
        ...
        */
        ...
        // An opaque no-cors response cannot confirm
        // server processing or email delivery.

        // Keep the entered details rather than
        // clearing an unconfirmed RFQ.
